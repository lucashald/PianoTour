...
ThreadController.prototype.init = function() {
	...
	// canvas
	this.cv = ctx;
	// Sound - Guitar samples (one per string)
	// Guitar tuning: EADGBE (from low to high)
	var guitarSamples = [
		'twang_e4_f_rr1.wav',  // String 0 - High E
		'twang_a4_f_rr1.wav',  // String 1 - A
		'twang_d4_f_rr1.wav',  // String 2 - D
		'twang_g4_f_rr1.wav',  // String 3 - G
		'twang_b4_f_rr1.wav',  // String 4 - B
		'twang_e3_f_rr1.wav'   // String 5 - Low E
	];

	// Create one player per guitar string
	for (var s = 0; s < guitarSamples.length; s++) {
		var player = new Tone.Player("./static/audio/" + guitarSamples[s]).toMaster();
		player.retrigger = true;
		player.volume.value = -9.0;
		this.arrPlayers[s] = player;
	}
	...
	var threadIndex = 0;  // Track unique thread index across all threads
	...
	for (var h = 0; h < HARMONICS; h++) {
		...
		for (var i = 0; i < howMany; i++) {
			...
			isTop = (i == 0);
			// Get the positions
			x0 = this.arrThreadPositions[threadIndex][0][0];
			y0 = this.arrThreadPositions[threadIndex][0][1];
			x1 = this.arrThreadPositions[threadIndex][1][0];
			y1 = this.arrThreadPositions[threadIndex][1][1];
			...
			var thr = new Thread(x0, y0, x1, y1, threadIndex, h, isTop, this);
