...
import { buildPianoSVG } from "./pianoChordDiagram.js";

// Print layout constants
export let displayChordName = true;
export function setDisplayChordName(value) {
  displayChordName = value;
}
export let displayPianoDiagrams = true;
export function setDisplayPianoDiagrams(value) {
  displayPianoDiagrams = value;
}
...
export async function drawAllPrint(containerId, measures) {
  ...
  if (typeof Vex === "undefined" || !Vex.Flow) {
    ...
    container.innerHTML =
      '<div class="message error">VexFlow library not loaded.</div>';
    return;
  }
      ...
      for (let i = 0; i < measuresInThisSystem; i++) {
        ...
        const trebleSpec = trebleNotesData.length
          ? trebleNotesData
              .map(
                (n) =>
                  `${n.name}/${getVexFlowDuration(n)}${n.isRest ? "/r" : ""}`,
              )
              .join(", ")
          : "B4/1/r";
            ...
            if (noteData.chordName && trebleVexNotes[idx]) {
              const annotation = new Vex.Flow.Annotation(noteData.chordName)
                ...
                .setVerticalJustification(
                  Vex.Flow.Annotation.VerticalJustify.BOTTOM,
                );
        ...
        if (pianoState.enableBeaming) {
          ...
          if (trebleNotesData.length > 0) {
            vexflowBeams[absoluteMeasureIndex].treble = createBeamsForNotes(
              trebleVexNotes,
              trebleNotesData,
            );
          }
          if (bassNotesData.length > 0) {
            vexflowBeams[absoluteMeasureIndex].bass = createBeamsForNotes(
              bassVexNotes,
              bassNotesData,
            );
          }
        ...
        const effectiveMeasureWidth =
          measuresInThisSystem < MEASURES_PER_LINE
        ...
        for (let i = 0; i < measuresInThisSystem; i++) {
          ...
          if (measureBeams && context) {
            if (measureBeams.treble && measureBeams.treble.length > 0) {
              measureBeams.treble.forEach((beam) =>
                beam.setContext(context).draw(),
              );
            }
            if (measureBeams.bass && measureBeams.bass.length > 0) {
              measureBeams.bass.forEach((beam) =>
                beam.setContext(context).draw(),
              );
            }
          }
  ...
  notesData.forEach((noteData) => {
    if (noteData.tie) {
      if (noteData.tie.startNoteId && noteData.tie.endNoteId) {
        ...
        const tieExists = tieGroups.some(
          (existingTie) =>
...
function drawTies(context, tieGroups, vexflowNoteMap, vexflowIndexByNoteId) {
  ...
  for (const noteId in vexflowIndexByNoteId) {
    ...
    const vexNote =
      vexflowNoteMap[noteInfo.measureIndex]?.[noteInfo.clef]?.[noteInfo.index];
    if (vexNote) {
...
function isChordSymbol(chordName) {
  ...
  if (/^[A-G][#b]*\d+$/.test(chordName)) return false; // single note name (e.g., "C4", "F#3")
  return true;
}
...
async function appendChordDiagramRow(container, measures) {
  // Collect { chordName, noteName } — noteName gives us the actual voiced notes
  // including any inversions, so we derive pitch classes from it directly.
  const seen = new Set();
  const chords = [];
  for (const measure of measures) {
    for (const note of measure) {
      if (isChordSymbol(note.chordName) && !seen.has(note.chordName)) {
        seen.add(note.chordName);
        chords.push({ chordName: note.chordName, noteName: note.name });
      }
    }
  }
  ...
  for (const { chordName, noteName } of chords) {
    try {
      const notes = noteName.startsWith("(")
        ? noteName.slice(1, -1).split(" ").filter(Boolean)
        : [noteName];
      const diagram = document.createElement("div");
      diagram.className = "chord-diagram";
      diagram.innerHTML = `<span class="chord-diagram-label">${chordName}</span>${buildPianoSVG(notes)}`;
