...
import { addNoteToMeasure, getDrumMeasures } from "./drumsScoreWriter.js";
...
let originalNoteData = null; // Store the original note's data from `getDrumMeasures()` for drag operation
...
function logNotePositions() {
  ...
  console.log("=== Actual rendered drum note positions ===");
  ...
  for (
    ...
    const measure = getDrumMeasures()[measureIndex];
    if (!measure) continue;
    ...
    measure.forEach((note, i) => {
      if (testNotes.includes(note.name) && !note.isRest) {
        ...
        if (vexNote) {
          ...
          console.log(
            `Drum ${note.name}: Y=${centerY} (bbox.y=${bbox.y}, height=${bbox.h})`
          );
        }
      }
    });
...
function calibrateStaffPositions() {
  console.log("Calibrating drum staff positions...");
  ...
  if (trebleStave) {
    ...
    console.log(
      `Drum treble staff: top=${TREBLE_STAFF_TOP_Y}, bottom=${TREBLE_STAFF_BOTTOM_Y}, spacing=${STAFF_LINE_SPACING_ACTUAL}`
    );
  }
  ...
  if (bassStave) {
    ...
    console.log(
      `Drum bass staff: top=${BASS_STAFF_TOP_Y}, bottom=${BASS_STAFF_BOTTOM_Y}`
    );
  }
  ...
  console.log(
    `Drum score vertical centering: container=${containerHeight}px, offset=${verticalOffset}px`
  );
}
...
function findNearestStaffPosition(y, clef) {
  ...
  if (!stave) {
    console.warn("No drum stave found for clef:", clef);
    return null;
  }
...
function calibrateTrebleStaff(stave) {
  try {
    ...
    console.log(
      `Drum treble staff calibration: top=${topLineY}, bottom=${bottomLineY}, height=${staffHeight}, calculated spacing=${STAFF_LINE_SPACING_ACTUAL}, G4 position=${TREBLE_CLEF_G4_Y}`
    );
  } catch (error) {
    console.error("Error calibrating drum treble staff:", error);
  }
}
...
function calibrateBassStaff(stave) {
  try {
    ...
    console.error("Error calibrating drum bass staff:", error);
  }
}
...
export function setKeySignature(keySignature) {
  if (!keySignature || typeof keySignature !== "string") {
    console.warn("drumRenderer setKeySignature: Invalid key signature provided");
    return false;
  }
  ...
  if (!keyData) {
    console.warn(`drumRenderer setKeySignature: Unknown key signature "${keySignature}"`);
    return false;
  }

  // Update the drum state
  drumsState.keySignature = keyData.displayName;
  drumsState.keySignatureType = keyData.type;
  ...
  // Log the change
  console.log(
    `Drum key signature set to: ${keyData.displayName} (type: ${keyData.type})`
  );
...
export function drawAll(measures) {
  ...
  const out = document.getElementById("drums-score");
  if (!out) {
  ...
  if (typeof Vex === "undefined" || !Vex.Flow) {
    console.error("drumRenderer drawAll: VexFlow library not loaded.");
    return;
  }

  try {
    // Calculate vertical centering
    const scoreContainer = out.parentElement; // Should be .drums-score-viewer__container
    const containerHeight = scoreContainer ? scoreContainer.clientHeight : 350;
    ...
    for (let i = 0; i < measureCount; i++) {
      ...
      function formatTimeSignature() {
        return `${drumsState.timeSignature.numerator}/${drumsState.timeSignature.denominator}`;
      }
      function formatTempo() {
        return drumsState.tempo;
      }
      ...
      if (i === 0) {
        ...
        staveTreble.setTempo({ duration: 'q', bpm: drumsState.tempo }, -27);
      }
      if (i === measureCount - 1) {
    ...
    if (drumsState.currentSelectedMeasure !== -1) {
      console.log(
        `drumRenderer drawAll: Restoring measure highlight for measure ${drumsState.currentSelectedMeasure}`
      );
      highlightSelectedMeasure(drumsState.currentSelectedMeasure);
    }
    if (drumsState.currentSelectedNote) {
      console.log(
        `drumRenderer drawAll: Restoring selected note highlight for note`,
        drumsState.currentSelectedNote
      );
      highlightSelectedNote(
        drumsState.currentSelectedNote.measureIndex,
        drumsState.currentSelectedNote.clef,
        drumsState.currentSelectedNote.noteId
      );
    }
    ...
    console.error("drumRenderer drawAll: VexFlow rendering error:", e);
  }
...
export function safeRedraw() {
  ...
  // drawAll now explicitly handles re-applying drumsState.currentSelectedMeasure, drumsState.currentSelectedNote, and drumsState.currentPlaybackNote
  drawAll(scoreData);
...
export function enableScoreInteraction(onMeasureClick, onNoteClick) {
  console.log("drumRenderer enableScoreInteraction: Attaching unified event listeners.");
  const scoreElement = document.getElementById("drums-score");
  if (!scoreElement) {
    console.error("drumRenderer enableScoreInteraction: Drum score element not found.");
    return;
  }
  ...
  scoreElement.addEventListener("mousedown", (event) => {
    ...
    console.log(`drumRenderer enableScoreInteraction: Mouse down at (${x}, ${y})`);
    ...
    if (mouseDownNoteTarget) {
      console.log(
        `drumRenderer enableScoreInteraction: Mouse down on target:`,
        mouseDownNoteTarget
      );
    }
  });
  ...
  scoreElement.addEventListener("mousemove", (event) => {
    ...
    console.log(`Drum drag coordinates: x=${currentX}, y=${currentY}`);
    ...
    if (mouseDownInitialPos && !isPaletteDrag && !isDraggingInitiated) {
      ...
      if (
        ...
        console.log("drumRenderer enableScoreInteraction: Drag initiated on existing note.");
      }
    }
    ...
    if (isPaletteDrag) {
      ...
      if (
        ...
        targetMeasureIndex !== drumsState.currentSelectedMeasure
      ) {
        highlightSelectedMeasure(targetMeasureIndex);
      } else if (
        targetMeasureIndex === -1 &&
        drumsState.currentSelectedMeasure !== -1
      ) {
        clearMeasureHighlight();
      }
  ...
  document.addEventListener("mouseup", (event) => {
    ...
    if (isOnScore && isDraggingInitiated) {
      ...
      console.log("drumRenderer enableScoreInteraction: Drag operation completed.");
      ...
      if (mouseDownNoteTarget && mouseDownNoteTarget.noteId !== null) {
        console.log("drumRenderer enableScoreInteraction: Pure click on EXISTING note detected.");
        onNoteClick(
        ...
        console.log("drumRenderer enableScoreInteraction: Pure click on measure background detected.");
      ...
      console.log("drumRenderer enableScoreInteraction: Drag canceled - mouse released outside score.");
    }
...
function handlePaletteDrop(endX, endY) {
  console.log("drumRenderer handlePaletteDrop: Processing palette drop at", endX, endY);
  clearDragPreview();

  const targetMeasureIndex = detectMeasureClick(endX, endY);
  if (targetMeasureIndex === -1) {
    console.log("drumRenderer handlePaletteDrop: Dropped outside a valid measure area.");
    return;
  }
  ...
  if (!nearestPosition) {
    console.warn(
      "drumRenderer handlePaletteDrop: Could not determine staff position for the drop."
    );
    return;
  }
  ...
  if (paletteDragType === "major" || paletteDragType === "minor") {
    ...
    console.log(
      `Drum chord drop (${paletteDragType}) detected. Creating single note for now.`
    );
  }

  // Find insertion position (simplified: for now, append to the clef)
  addNoteToMeasure(targetMeasureIndex, newNote);
  console.log("drumRenderer handlePaletteDrop: Added new note to measure:", newNote);
}
...
function updateDragPreview(x, snapY, noteName) {
  ...
  let preview = document.getElementById("drum-drag-snap-preview");
  ...
  if (!preview) {
    ...
    preview.id = "drum-drag-snap-preview";
    ...
    `;
    document.getElementById("drums-score").appendChild(preview);
  }
...
function clearDragPreview() {
  const preview = document.getElementById("drum-drag-snap-preview");
  if (preview) {
    preview.style.display = "none";
    console.log("drumRenderer clearDragPreview: Preview cleared");
  }
}

function startDrag(noteInfo, initialClickPos) {
  console.log("drumRenderer startDrag: Initializing drag state.");
  draggedNote = noteInfo;
  dragStartPosition = initialClickPos;
  const measures = getDrumMeasures();
  ...
  if (foundNote) {
    ...
    console.warn(
      "drumRenderer startDrag: Original note data not found in measures model. Using fallback data."
    );
    originalNoteData = {
      name: "C4",
      clef: "treble",
  ...
  if (vexNote) {
    ...
    console.warn(
      "drumRenderer startDrag: VexFlow note for BBox not found. BBox fallback active."
    );
  ...
  console.log("drumRenderer startDrag: Drag state initialized for note:", draggedNote);
}

function completeDrag(currentX, currentY) {
  console.log("drumRenderer completeDrag: Processing drag completion.");
  ...
  if (!draggedNote || !originalNoteData) {
    console.warn(
      "drumRenderer completeDrag: Called without active dragged note or original data. Aborting."
    );
    return;
  }
  ...
  if (targetMeasureIndex === -1) {
    console.log(
      "drumRenderer completeDrag: Dropped outside valid measure area. Drag operation cancelled."
    );
    return;
  }
  ...
  if (potentialNewClef && potentialNewClef !== originalNoteData.clef) {
    ...
    console.log(
      `drumRenderer completeDrag: Clef change detected: from ${originalNoteData.clef} to ${newClef}`
    );
  }
  ...
  if (!originalNoteData.isRest && originalVexFlowNoteBBox) {
    ...
    if (newNoteInfo) {
      ...
      console.warn(
        `drumRenderer completeDrag: Could not find note name for MIDI ${calculatedNewPitchMIDI}. Keeping original pitch name as fallback.`
      );
    ...
    if (
      ...
      pitchChanged = true;
      console.log(
        `drumRenderer completeDrag: Pitch change detected: from ${originalNoteData.name} to ${newPitchName} in ${newClef} clef.`
      );
    }
  } else if (originalNoteData.isRest) {
    newPitchName = "R";
    console.log("drumRenderer completeDrag: Dragged a rest. Pitch name remains R.");
  }

  let insertBeforeNoteId = null;
  const currentMeasuresData = getDrumMeasures();
    ...
    if (vexFlowNotesInTargetClef) {
      for (let i = 0; i < vexFlowNotesInTargetClef.length; i++) {
        ...
        if (
          ...
          console.log(
            `drumRenderer completeDrag: Determined insertion before note ID ${insertBeforeNoteId} at VexFlow clef index ${i}.`
          );
          break;
        }
      }
  ...
  console.log(
    "drumRenderer completeDrag: Insert before note ID:",
    insertBeforeNoteId || "Append to end"
  );

  document.dispatchEvent(
    new CustomEvent("drumNoteDropped", {
      detail: {
  ...
  console.log("drumRenderer completeDrag: Dispatched drumNoteDropped event with details:", {
    fromMeasureIndex: draggedNote.measureIndex,
    fromNoteId: draggedNote.noteId,
    toMeasureIndex: targetMeasureIndex,
    insertBeforeNoteId: insertBeforeNoteId,
...
function calculateAbsolutePitchFromY(y, clef) {
  ...
  if (!nearest) {
    console.warn("drumRenderer: Could not find nearest staff position, using fallback");
    return 60; // Middle C fallback
  }

  console.log(
    `drumRenderer: Snapped Y=${y} to position ${nearest.position} (${nearest.note}) at Y=${nearest.y}`
  );

  // Convert note name to MIDI
  const midi = NOTES_BY_NAME[nearest.note];
  if (midi === undefined) {
    console.error("drumRenderer: Unknown note name:", nearest.note);
    return 60;
  }
...
function calculatePitchChange(
  ...
  if (originalPitchMIDI === undefined) {
    console.warn(
      `drumRenderer calculatePitchChange: Original note MIDI not found for ${originalNoteName}. Cannot calculate accurate pitch change.`
    );
    return {
      originalPitch: -1,
      newPitch: -1,
...
function convertVexFlowIndexToLinearIndex(measureIndex, clef, vexflowIndex) {
  const measuresData = getDrumMeasures(); // Get the most up-to-date measures data
  const measureData = measuresData[measureIndex];
  if (!measureData) {
    console.warn(
      `drumRenderer convertVexFlowIndexToLinearIndex: No measure data found for index ${measureIndex}.`
    );
    return -1;
  }
  ...
  console.warn(
    `drumRenderer convertVexFlowIndexToLinearIndex: Could not find linear index for VexFlow index ${vexflowIndex} in measure ${measureIndex}, clef ${clef}.`
  );
  return -1;
}
...
function convertLinearIndexToVexFlowIndex(measureIndex, clef, linearIndex) {
  const measureData = getDrumMeasures()[measureIndex];
...
function detectNoteClick(x, y) {
  console.log("drumRenderer detectNoteClick: Input x:", x, "y:", y);
  const measuresData = getDrumMeasures();
  for (
    let measureIndex = 0;
    ...
    for (let i = 0; i < currentMeasureData.length; i++) {
      ...
      if (
        ...
        if (
          ...
          console.log("drumRenderer detectNoteClick: Existing note found and mapped:", {
            measureIndex,
            clef,
            noteId,
  ...
  if (clickedMeasureIndex !== -1) {
    ...
    console.log(
      `drumRenderer detectNoteClick: Clicked on Measure Background: Measure ${clickedMeasureIndex}, Clef Region ${clefRegion}.`
    );
  ...
  console.log(
    "drumRenderer detectNoteClick: No note or measure background found at given coordinates."
  );
  return null;
}
...
function detectMeasureClick(x, y) {
  ...
  if (y < scoreTopY - topMargin || y > scoreBottomY + bottomMargin) {
    console.log(
      `drumRenderer detectMeasureClick: Y=${y} outside bounds (${scoreTopY - topMargin} to ${
...
console.log("✓ drumRenderer.js loaded successfully");