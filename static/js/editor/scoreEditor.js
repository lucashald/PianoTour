...
function parseChord(chordString) {
  ...
  return chordString
    .substring(1, chordString.length - 1)
    .split(" ")
    .filter(Boolean);
}
...
function transposeChord(chordString, semitones) {
  ...
  const transposedNotes = notes.map((noteName) => {
    ...
    if (originalMidi === undefined) {
      console.warn(
        `transposeChord: Unknown note ${noteName}, keeping unchanged`,
      );
      return noteName;
    }
    ...
    if (!newNoteInfo) {
      console.warn(
        `transposeChord: MIDI ${newMidi} out of range, keeping ${noteName} unchanged`,
      );
      return noteName;
    }
...
function findNextNoteInClef(measureIndex, noteId, samePitchOnly = false) {
  ...
  const currentNoteIndex = currentMeasure.findIndex(
    (note) => note.id === noteId,
  );
...
function handleAddTie() {
  ...
  const selectedNote = currentMeasure.find(
    (note) => note.id === editorSelectedNoteId,
  );
  if (!selectedNote) {
  ...
  const targetNote = findNextNoteInClef(
    editorSelectedMeasureIndex,
    editorSelectedNoteId,
    true,
  );
  ...
  if (targetNote) {
    ...
    if (success) {
      console.log(
        `Tie created from ${editorSelectedNoteId} to ${targetNote.noteId}`,
      );
    ...
    console.log(
      "No suitable note found to tie to (must be same pitch in same clef)",
    );
  }
}
...
function handleAddSlur() {
  ...
  if (!slurMode) {
    ...
    if (slurStartNoteId && slurStartNoteId !== editorSelectedNoteId) {
      ...
      if (success) {
        console.log(
          `Slur created from ${slurStartNoteId} to ${editorSelectedNoteId}`,
        );
        renderNoteEditBox(false);
      } else {
...
function handleAddAutoSlur() {
  ...
  if (targetNote) {
    ...
    if (success) {
      console.log(
        `Auto slur created from ${editorSelectedNoteId} to ${targetNote.noteId}`,
      );
      renderNoteEditBox(false);
    } else {
...
function transposeSelectedNote(semitones) {
  ...
  // Update the note
  updateNoteInMeasure(editorSelectedMeasureIndex, editorSelectedNoteId, {
    name: transposedName,
  });
  renderNoteEditBox(false);
}
...
function invertSelectedNote(direction) {
  ...
  const measures = getMeasures();
  const note = measures[editorSelectedMeasureIndex]?.find(
    (n) => n.id === editorSelectedNoteId,
  );
...
// Score-level items shown when right-clicking the title, whitespace, or outside measures.
function addScoreLevelItems(menu) {
  ...
  addMenuItem(menu, "Set Time Signature", () => {
    const current = `${pianoState.timeSignature.numerator}/${pianoState.timeSignature.denominator}`;
    const input = window.prompt("Time signature (e.g. 4/4, 3/4):", current);
    if (input === null) return;
    const match = input.trim().match(/^(\d+)\s*\/\s*(\d+)$/);
    if (!match) {
      showToast("Invalid time signature. Use format like 4/4 or 3/4", {
        type: "error",
      });
      return;
    }
    ...
    if (!setTimeSignature(n, d)) {
      showToast("Invalid time signature", { type: "error" });
    }
  });

  addMenuItem(menu, "Set Tempo", () => {
    const input = window.prompt("Tempo (BPM, 30–300):", pianoState.tempo);
    if (input === null) return;
    const bpm = parseInt(input, 10);
    if (!setTempo(bpm)) {
      showToast("Invalid tempo. Enter a number between 30 and 300", {
        type: "error",
      });
    }
  });
...
const SHARP_KEYS = [
  { key: "C", major: "C Major", minor: "A Minor" },
  { key: "G", major: "G Major", minor: "E Minor" },
  { key: "D", major: "D Major", minor: "B Minor" },
  { key: "A", major: "A Major", minor: "F# Minor" },
  { key: "E", major: "E Major", minor: "C# Minor" },
  { key: "B", major: "B Major", minor: "G# Minor" },
  { key: "F#", major: "F# Major", minor: "D# Minor" },
  { key: "C#", major: "C# Major", minor: "A# Minor" },
];
const FLAT_KEYS = [
  { key: "F", major: "F Major", minor: "D Minor" },
  { key: "Bb", major: "B♭ Major", minor: "G Minor" },
  { key: "Eb", major: "E♭ Major", minor: "C Minor" },
  { key: "Ab", major: "A♭ Major", minor: "F Minor" },
  { key: "Db", major: "D♭ Major", minor: "B♭ Minor" },
  { key: "Gb", major: "G♭ Major", minor: "E♭ Minor" },
  { key: "Cb", major: "C♭ Major", minor: "A♭ Minor" },
];

function addKeySubmenu(menu) {
  const wrapper = document.createElement("div");
  wrapper.className = "editor-context-menu__submenu-wrapper";

  const trigger = document.createElement("div");
  trigger.className =
    "editor-context-menu__item editor-context-menu__item--has-submenu";
  trigger.textContent = "Set Key";
  wrapper.appendChild(trigger);

  const submenu = document.createElement("div");
  submenu.className = "editor-context-menu__submenu";

  // Major / Minor toggle — mirrors key-mode-toggle-btn in the dropdown
  const toggleItem = document.createElement("div");
  toggleItem.className = "editor-context-menu__item";
  toggleItem.style.cssText =
    "justify-content: center; border-bottom: 1px solid rgba(255,255,255,0.1); font-weight: 600;";
  const toggleText = document.createElement("span");
  toggleText.textContent = pianoState.isMinorKey ? "Minor" : "Major";
  toggleItem.appendChild(toggleText);
  toggleItem.addEventListener("click", (e) => {
    e.stopPropagation();
    toggleIsMinorKey(); // flips pianoState.isMinorKey and calls updateKeyMenuActiveState(),
    // which updates all .key-option/.key-name elements including ours
    toggleText.textContent = pianoState.isMinorKey ? "Minor" : "Major";
  });
  submenu.appendChild(toggleItem);

  // Helper to append a labelled section of key items
  const addSection = (label, keys) => {
    const sectionHeader = document.createElement("div");
    sectionHeader.className = "editor-context-menu__header";
    submenu.appendChild(sectionHeader);
    sectionHeader.textContent = label;

    keys.forEach(({ key, major, minor }) => {
      const item = document.createElement("div");
      // .key-option makes updateKeyMenuActiveState() keep labels in sync
      item.className = "editor-context-menu__item key-option";
      item.dataset.key = key;
      item.dataset.major = major;
      item.dataset.minor = minor;

      const nameSpan = document.createElement("span");
      nameSpan.className = "key-name";
      nameSpan.textContent = pianoState.isMinorKey ? minor : major;
      item.appendChild(nameSpan);

      const check = document.createElement("span");
      check.textContent = "✓";
      check.style.cssText = `color: #94a3b8; display: ${key === pianoState.keySignature ? "inline" : "none"};`;
  ...
  addSection("Sharp Keys", SHARP_KEYS);
  addSection("Flat Keys", FLAT_KEYS);
...
function buildNoteContextMenu(menu, detail) {
  ...
  addMenuItem(menu, note.isRest ? "Make Note" : "Make Rest", () => {
    ...
    placeNote(editorSelectedMeasureIndex, note.id, editorSelectedMeasureIndex, {
      isRest: newIsRest,
    });
    renderNoteEditBox(false);
  });
  ...
  addMenuItem(menu, "Insert Note After", () => {
    ...
    const insertBeforeId =
      noteIndex < currentMeasure.length - 1
        ? currentMeasure[noteIndex + 1].id
        : null;
    const newNote = {
      name: "C4",
      clef: note.clef,
...
function addSubmenu(menu, label, items, options = {}) {
  ...
  submenu.className = "editor-context-menu__submenu";
  if (options.openUp)
    submenu.classList.add("editor-context-menu__submenu--open-up");
...
function renderNoteEditBox(smoothScroll = true) {
  ...
  if (selectedNote) {
    ...
    const displayName = selectedNote.isRest
      ? "Rest"
      : selectedNote.chordName || selectedNote.name;
...
function renderSidebarNoteList(currentMeasure, selectedNote) {
  ...
  document.getElementById("editorPrevBtn").disabled =
    editorSelectedMeasureIndex <= 0;
  document.getElementById("editorNextBtn").disabled = false;

  const trebleNotesContainer = document.getElementById(
    "editorTrebleNotesContainer",
  );
  const bassNotesContainer = document.getElementById(
  ...
  initialTrebleWrapper.appendChild(
    createAddButton("treble", trebleNotes[0]?.id || null),
  );
  ...
  initialBassWrapper.appendChild(
    createAddButton("bass", bassNotes[0]?.id || null),
  );
...
function renderSidebarNoteEditor(currentMeasure, selectedNote) {
  ...
  if (selectedNote) {
    ...
    document.getElementById("editorSelectedNoteDisplay").textContent =
      isSelectedNoteRest
        ? "Rest"
        : isSelectedNoteChord
          ? `Chord (${parseChord(selectedNote.name).length})`
          : selectedNote.name;
    document.getElementById("editorToggleClef").textContent =
      selectedNote.clef === "treble" ? "Treble" : "Bass";
    document.getElementById("editorDurationDropdown").value =
      selectedNote.duration;

    // Update performed duration slider (stored as 0-1, displayed as 0-100%)
    const performedDurationSlider = document.getElementById(
      "editorPerformedDuration",
    );
    const performedDurationValue = document.getElementById(
      "editorPerformedDurationValue",
    );
    ...
    if (isSelectedNoteRest) {
      ...
      individualChordNotes.forEach((noteName, noteIndex) => {
        ...
        noteDiv.querySelector(".chord-note-accidental").value =
      ...
      singleNoteControls.classList.remove("hidden");
      chordNotesEditor.classList.add("hidden");
      const { letter, accidental, octave } = parseSingleNoteName(
        selectedNote.name,
      );
...
function changeMeasure(newMeasureIndex, clearSelectedNote = true) {
  ...
  const measureDisplay = document.getElementById(
    "editorSelectedMeasureDisplay",
  );
  if (measureDisplay) {
...
function updateChordFromUI() {
  ...
  const chordNotes = [];
  document
    .querySelectorAll("#chordNotesContainer .chord-note-item")
    .forEach((item) => {
  ...
  if (chordNotes.length === 0) {
    ...
    placeNote(
      ...
      selectedNote.id,
      editorSelectedMeasureIndex,
      { name: chordNotes[0], isRest: false },
    );
  } else {
    placeNote(
      editorSelectedMeasureIndex,
      selectedNote.id,
      editorSelectedMeasureIndex,
      { name: formatChord(chordNotes), isRest: false },
    );
  }
  renderNoteEditBox(false);
}
...
function addNoteToChordUI() {
  ...
  if (!selectedNote) return;
  let currentChordNotes = isChord(selectedNote.name)
    ? parseChord(selectedNote.name)
    : [selectedNote.name];
  ...
  placeNote(
    ...
    selectedNote.id,
    editorSelectedMeasureIndex,
    { name: formatChord(currentChordNotes), isRest: false },
  );
  renderNoteEditBox(false);
}
...
function removeNoteFromChordUI(noteIndexToRemove) {
  ...
  if (currentChordNotes.length > 1) {
    ...
    placeNote(
      ...
      selectedNote.id,
      editorSelectedMeasureIndex,
      { name: formatChord(currentChordNotes) },
    );
  } else {
    placeNote(
      editorSelectedMeasureIndex,
...
export function initializeScoreInteraction() {
  ...
  enableScoreInteraction(
    (measureIndex, wasNoteClicked) =>
      changeMeasure(measureIndex, !wasNoteClicked),
    handleEditorNoteSelectClick,
  );
  ...
  document.addEventListener("noteDropped", (event) => {
    ...
    if (droppedData && droppedData.startsWith("application/chord:")) {
      try {
        const chordData = JSON.parse(
          droppedData.replace("application/chord:", ""),
        );
        ...
        console.log(
          `Added ${chordDisplayName} chord to ${targetClef} clef:`,
          notesToUse,
        );
        return;
      } catch (error) {
    ...
    const measures = getMeasures();
    const originalNote = measures[fromMeasureIndex]?.find(
    ...
    if (fromMeasureIndex !== toMeasureIndex) {
        ...
        if (pitchChanged && !originalNote.isRest) {
          ...
          if (isChord(originalNote.name)) {
            ...
            if (anchorNoteMidi !== undefined && newPitchMidi !== undefined) {
              ...
              noteToMove.name = transposeChord(
                originalNote.name,
                semitoneChange,
              );
            }
      ...
      if (pitchChanged && !originalNote.isRest) {
        if (isChord(originalNote.name)) {
          ...
          if (anchorNoteMidi !== undefined && newPitchMidi !== undefined) {
            const semitoneChange = newPitchMidi - anchorNoteMidi;
            updatedNoteData.name = transposeChord(
              originalNote.name,
              semitoneChange,
            );
          }
  ...
  document.addEventListener("scoreContextMenu", (event) => {
    const { clientX, clientY, noteTarget, measureIndex, clickRegion } =
      event.detail;
    console.log("context-menu: scoreContextMenu received", {
      clickRegion,
      noteTarget,
      measureIndex,
    });
...
export function initializeEditorSidebar() {
  ...
  if (!editorContainer) {
    console.error(
      "context-menu: Editor container with ID 'editorContainer' not found. Cannot initialize sidebar.",
    );
    return;
  }
  ...
  editorContainer.addEventListener("click", (event) => {
    ...
    if (target.classList.contains("add-note-initial")) {
      const insertBeforeNoteId =
        target.dataset.insertBeforeNoteId === "null"
          ? null
          : target.dataset.insertBeforeNoteId;
      ...
      const addedNoteResult = addNoteToMeasure(
        editorSelectedMeasureIndex,
        newNote,
        insertBeforeNoteId,
      );
      ...
      if (addedNoteResult) {
        // Automatically select the newly added note
        handleEditorNoteSelectClick(
          addedNoteResult.measureIndex,
          addedNoteResult.clef,
          addedNoteResult.noteId,
        );
      }
    }
  ...
  editorContainer.addEventListener("change", (event) => {
    ...
    if (
      target.id === "time-signature-numerator" ||
      target.id === "time-signature-denominator"
    ) {
      const numerator = parseInt(
        document.getElementById("time-signature-numerator").value,
        10,
      );
      const denominator = parseInt(
        document.getElementById("time-signature-denominator").value,
        10,
      );
    ...
    if (
      target.matches(
        ".chord-note-letter, .chord-note-accidental, .chord-note-octave",
      )
    ) {
      updateChordFromUI();
      return;
    ...
    if (
      [
        "editorNoteLetter",
        "editorAccidentalDropdown",
        "editorOctaveDropdown",
      ].includes(target.id)
    ) {
      ...
      if (newLetter === "R") {
        ...
        let newAccidental = document.getElementById(
          "editorAccidentalDropdown",
        ).value;
...
export function initializeMusicEditor() {
  console.log(
    "context-menu: initializeMusicEditor() called — initializing both score interaction and sidebar",
  );
