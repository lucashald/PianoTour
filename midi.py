...
def process_measures(measures, tempo, time_signature):
    ...
    for measure_idx, measure in enumerate(measures):
        measure_duration_seconds = beats_to_seconds(measure_duration_beats, tempo)
        ...
        for note_data in measure_notes:
            ...
            try:
                ...
                print(f"Warning: Could not parse note '{note_data['name']}': {e}")
                continue
...
def get_instrument_program(instrument_name):
    ...
    instrument_display_name = instrument_map.get(instrument_name.lower(), 'Acoustic Grand Piano')
...
def create_midi_from_multiple_json(json_files_data, output_tempo=None):
    ...
    time_signature = json_files_data[0].get('timeSignature', {'numerator': 4, 'denominator': 4})
    ...
    for file_idx, json_data in enumerate(json_files_data):
        instrument_name = json_data.get('instrument', f'instrument_{file_idx + 1}')
        ...
        while channel in used_channels:
            ...
            if channel >= 16:  # MIDI only has 16 channels
                print(f"Warning: Too many instruments, wrapping channel assignments")
                channel = 0
                break
        used_channels.add(channel)

        if channel != requested_channel:
            print(f"Channel {requested_channel} already used, assigned channel {channel} to {instrument_name}")
...
def create_json_from_midi(midi_file_path, quantize_resolution=0.25):
    ...
    if main_instrument:
        try:
            program_name = pretty_midi.program_to_instrument_name(main_instrument.program)
    ...
    beats_per_measure = time_signature['numerator'] * (4.0 / time_signature['denominator'])
    ...
    if all_notes:
        ...
        for measure_idx in range(max_measure + 1):
            ...
            for note in measure_notes:
                ...
                if current_time is None or abs(note['start_time'] - current_time) < 0.1:
            ...
            for group in chord_groups:
                ...
                for clef, clef_notes in clef_groups.items():
                    ...
                    avg_duration = sum(n['duration_beats'] for n in clef_notes) / len(clef_notes)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Convert VexFlow JSON to MIDI file',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    # Single instrument
    python vexflow_to_midi.py song.json song.mid

    # Multiple instruments (symphony/ensemble)
    python vexflow_to_midi.py piano.json guitar.json drums.json --output ensemble.mid
    python vexflow_to_midi.py *.json --output symphony.mid
        """
    )
    parser.add_argument('inputs', nargs='+', help='Input JSON file(s)')
    parser.add_argument('output', nargs='?', help='Output MIDI file (required if multiple inputs)')
    parser.add_argument('--output', '-o', dest='output_flag', help='Output MIDI file')
    parser.add_argument('--tempo', type=int, help='Override tempo (BPM) for all instruments')
    ...
    for input_file in input_files:
        ...
        try:
            ...
            if args.verbose:
                ...
                print(f"Loaded {input_file}: {instrument} ({measures} measures)")
