...
currentMeasure.forEach((note, noteIndex) => { // Keep noteIndex for iteration purposes
...
noteWrapper.appendChild(createNoteButton(note)); // Pass the whole note object
noteWrapper.appendChild(createAddButton(note.clef, noteIndex)); // Use noteIndex for insertion position
...
individualChordNotes.forEach((noteName, noteIndex) => { // Use noteIndex for iterating within chord
...
currentChordNotes.splice(noteIndexToRemove, 1); // Remove the note
...
editorSelectedNoteId = addedNoteId; // Select the newly added note by its ID
renderNoteEditBox();
}
...
editorSelectedNoteId = null; // Deselect after removal
renderNoteEditBox();
}
...
console.log(`Attempting to move note with ID ${editorSelectedNoteId} from measure ${editorSelectedMeasureIndex} to ${editorSelectedMeasureIndex - 1}`);
moveNoteBetweenMeasures(editorSelectedMeasureIndex, editorSelectedNoteId, editorSelectedMeasureIndex - 1);
editorSelectedNoteId = null; // Deselect after move
renderNoteEditBox();
} else {
...
const selectedNote = currentMeasure.find(note => note.id === editorSelectedNoteId);
if (!selectedNote) {
console.warn('Selected note not found by ID for change event:', editorSelectedNoteId);
return;
}
...
// We need to re-evaluate editorSelectedNoteId as the note's position changed
editorSelectedNoteId = noteToMove.id; // Select the moved note by its ID
...
editorSelectedNoteId = fromNoteId; // Keep the note selected if it's an in-place edit
