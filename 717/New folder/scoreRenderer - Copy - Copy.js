...
// Track highlighted notes to avoid redundant redraws / to manage state
// These sets will now track notes that need their styles restored after selection/deselection
let notesHighlightedInMeasure = new Set(); // Stores noteKeys (e.g., "0-treble-0") for notes highlighted in green
let notesHighlightedForPlayback = new Set(); // Stores noteKeys for notes highlighted during playback

// Store original note styles for restoration after a redraw or deselection
let originalNoteStyles = new Map(); // Stores original {fillStyle, strokeStyle} for noteKeys

// Variables for selected measure/note highlighting
let currentSelectedMeasure = -1; // Stores the index of the currently selected measure (for black box)
...
export function drawAll(measures) {
    ...
    // Capture current highlights before clearing for redraw
    const measureToRestoreHighlight = currentSelectedMeasure;
    const noteToRestoreHighlight = currentSelectedNote;

    // Clear previous rendering state and existing highlight states
    out.innerHTML = ''; // Clear the canvas DOM element
    vexflowNoteMap = []; // This will be rebuilt by EasyScore/Factory.draw()
    measureXPositions = [];
    vexflowStaveMap = [];
    
    // Clear DOM-based measure highlight overlay
    clearMeasureHighlight();
    
    // NOTE: individual VexFlow note styles and classes are reset by the full redraw.
    // We will re-apply them after the draw.
    ...
    try {
        ...
        // Restore note highlights for the measure (green) and selected note (orange)
        // These calls will now correctly use setStyle and drawWithStyle
        if (measureToRestoreHighlight !== -1) {
            // Re-identify notes in the measure for highlighting purposes
            const measureNotes = [];
            if (vexflowNoteMap[measureToRestoreHighlight].treble) {
                vexflowNoteMap[measureToRestoreHighlight].treble.forEach((note, idx) => {
                    measureNotes.push({ clef: 'treble', noteIndex: idx });
                });
            }
            if (vexflowNoteMap[measureToRestoreHighlight].bass) {
                vexflowNoteMap[measureToRestoreHighlight].bass.forEach((note, idx) => {
                    measureNotes.push({ clef: 'bass', noteIndex: idx });
                });
            }

            measureNotes.forEach(({ clef, noteIndex }) => {
                // Apply green highlight to all notes in the selected measure
                addHighlight(measureToRestoreHighlight, clef, noteIndex, '#1db954', false);
            });
        }

        if (noteToRestoreHighlight) {
            // Apply orange highlight to the specifically selected note
...
export function enableScoreInteraction(onMeasureClick, onNoteClick) {
    ...
    // Use 'mousedown' for primary selection logic
    scoreElement.addEventListener('mousedown', (event) => {
        console.log('Score element mousedown event. Event:', event);
        // Reset isDragging flag at the start of every potential click/drag sequence
        isDragging = false;
        ...
        // 1. Check if mousedown is on a note first (most specific)
        ...
        if (noteInfo) {
            ...
            // We do NOT preventDefault here, to allow drag-and-drop to initiate.
            // Selection will effectively be "confirmed" on mouseup if no drag.
            return; 
        }
