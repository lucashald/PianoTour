...
import { safeRedraw, addPlaybackHighlight, removePlaybackHighlight, clearAllHighlights, scrollToMeasure } from './scoreRenderer.js'; // For visual score updates and scrolling
 ...
 * @param {number} noteIndex - The index of the note within its clef's notes for the measure.
 ...
 */
function scheduleNoteEvents(note, measureIndex, noteIndex, currentTransportTime, clefOffset, secondsPerBeat) {
    ...
    const noteKey = `${measureIndex}-${note.clef}-${noteIndex}`; // Unique key for Set
    ...
    if (!note.isRest) {
        ...
        Tone.Transport.scheduleOnce(time => {
            Tone.Draw.schedule(() => {
                addPlaybackHighlight(measureIndex, note.clef, noteIndex, PLAYBACK_HIGHLIGHT_COLOR);
        ...
        Tone.Transport.scheduleOnce(time => {
            Tone.Draw.schedule(() => {
                removePlaybackHighlight(measureIndex, note.clef, noteIndex);
