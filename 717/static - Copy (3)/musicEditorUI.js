...
// ===================================================================
function handleEditorNoteSelectClick(measureIndex, clef, noteIndex) {
    // IMPORTANT: The `noteIndex` parameter passed here is now the LINEAR index from scoreRenderer.detectNoteClick,
    // or -1 if an empty space/measure background was clicked. It is NOT the VexFlow index anymore.
    console.log('handleEditorNoteSelectClick called with measure:', measureIndex, 'clef:', clef, 'noteIndex (linear):', noteIndex);
    ...
    if (noteIndex === -1) {
        ...
        editorSelectedNoteIndex = null; // Clear any existing note selection
    ...
    // If we reach here, `noteIndex` is a valid linear index for an existing note.
    const currentMeasureNotes = getMeasures()[measureIndex] || [];
    
    // Validate that the note actually exists at this linear index
    if (noteIndex < 0 || noteIndex >= currentMeasureNotes.length || currentMeasureNotes[noteIndex].clef !== clef) {
        console.warn(`handleEditorNoteSelectClick: Invalid or mismatched note data for linear index ${noteIndex} in measure ${measureIndex}, clef ${clef}. Deselecting note.`);
        editorSelectedNoteIndex = null;
        clearSelectedNoteHighlight();
        renderNoteEditBox();
        return;
    ...
    editorSelectedNoteIndex = (editorSelectedNoteIndex === noteIndex) ? null : noteIndex;
